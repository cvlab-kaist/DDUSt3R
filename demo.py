...
def main_demo(tmpdirname, model, device, image_size, server_name, server_port, silent=False, args=None):
    ...
    with gradio.Blocks(css=""".gradio-container {margin: 0 !important; min-width: 100%};""", title="Demo") as demo:
        # scene state is save so that you can change conf_thr, cam_size... without rerunning the inference
        scene = gradio.State(None)
        gradio.HTML(f'<h2 style="text-align: center;">DDUSt3R Demo</h2>')
        with gradio.Column():
...
if __name__ == '__main__':
    ...
    tmpdirname = args.output_dir if args.output_dir is not None else tempfile.mkdtemp(suffix='gradio_demo')
